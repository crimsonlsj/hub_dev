...
window.liquidHeaderIsElementor = $liquidMainHeader.children('.elementor').length;
...
class LiquidSectionsDetails {
  constructor() {
    const headerStickyData = $liquidMainHeader.data('stickyOptions');
    this.shouldAddLuminosity = $liquidContents[0].hasAttribute('data-liquid-stack') || headerStickyData && headerStickyData.dynamicColors;
    this.sections = [];
    this.instance = null;
  ...
  async getDetails() {
    const liquidContentsRect = await this.getElementRect({
      element: $liquidContents[0]
    });
    const sections = await Promise.all(this.init(liquidContentsRect));
    this.addParentSections();
    this.addInnerSections();
    this.addLuminosity();
    return sections;
  }

  init(liquidContentsRect) {
    const promises = [];
  ...
  addLuminosity() {
    ...
    sections.forEach(async sec => {
      ...
      sec.luminosity = sec.predefinedLuminosity ? sec.predefinedLuminosity : await this.getLuminosity(sec);
      fastdom.mutate(() => {
  ...
  addParentSections() {
    const sections = [...this.sections];
    const innerSections = sections.filter(sec => sec.isInnerSection);
  ...
  class Plugin {
    ...
    handleToggle(state, event) {
      ...
      if (submenu.length) {
        ...
        if (toggleType === 'fade' && state === 'show') {
          ...
          this.fadeOut(submenu, isMegamenu, megamenuScheme, megamenuBg);
        }
        ...
        if (toggleType === 'slide' && state === 'show') {
          ...
          this.slideUp(submenu, isMegamenu, megamenuScheme, megamenuBg);
        }
    ...
    fadeToggle(submenu, isMegamenu, megamenuScheme, megamenuBg) {
      if (isMegamenu) {
        ...
        $liquidMainHeader.removeClass('megamenu-scheme-dark megamenu-scheme-light');
...
(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      if (this.options.disableOnMobile && liquidIsMobile()) return;
      this.DOM = {};
      this.DOM.element = element;
      this.DOM.$element = $(element);
      this.DOM.$stickySections = liquidIsElementor ? $('> .elementor > .elementor-section-wrap > .elementor-section', element).not('.lqd-hide-onstuck, .lqd-stickybar-wrap') : $('.lqd-head-sec-wrap', element).not('.lqd-hide-onstuck');
    ...
    eventListeners() {
      $liquidMainHeader.on('animationend', async event => {
        if (event.target && $(event.target).is($liquidMainHeader) && this.recalcStickyElsPos) {
          await this.getStickyElsDetails();
          this.recalcStickyElsPos = false;
        }
      });
    ...
    updateStickyStates(isStuck) {
      fastdom.mutate(() => {
        ...
        if (!this.isOverlay || this.isInTitlebar) {
          this.DOM.placeholder.classList.toggle('hide', !this.isStuck);
          this.DOM.placeholder.classList.toggle('d-none', !this.isStuck);
        }

        ;
        this.recalcStickyElsPos = true;
    ...
    addPlaceholder() {
      return fastdomPromised.mutate(() => {
        ...
        placeholder.setAttribute('class', 'lqd-sticky-placeholder hide d-none');
    ...
    addSentinel() {
      return fastdomPromised.mutate(() => {
        ...
        sentinel.setAttribute('class', 'lqd-sticky-sentinel invisible pos-abs pointer-events-none');
    ...
    headerIO() {
      if (this.observer) {
        this.observer.unobserve(this.DOM.element);
        this.observer = null;
      }
        ...
        fastdom.measure(() => {
          ...
          fastdom.mutate(() => {
            if (stickyTrigger === 'this') {
              this.DOM.sentinel.style.top = `${targetInfo.top + targetInfo.height + window.scrollY}px`;
            }

            this.DOM.placeholder.style.height = `${targetInfo.height}px`;
            observer.disconnect();
          });
        });
      });
    ...
    stickyVisibilityChange(state) {
      const recalcStickyElsPos = state === 'show';
      ...
      this.recalcStickyElsPos = recalcStickyElsPos;
    ...
    getStickyElements() {
      const $stickyModules = liquidIsElementor ? this.DOM.$element.find('> .elementor > .elementor-section-wrap > .elementor-section').not('.lqd-hide-onstuck').find('[data-element_type="widget"]') : this.DOM.$element.find('.lqd-head-sec-wrap, .lqd-stickybar-wrap').not('.lqd-hide-onstuck').find('.lqd-head-col > .header-module');
        ...
        const promise = new Promise(resolve => {
          new IntersectionObserver(([entry], observer) => {
            fastdom.measure(() => {
              ...
              if (x < 0) {
                x = 0;
              } else if (x >= rootBounds.width) {
                x = rootBounds.width - width - entry.target.parentElement.offsetWidth;
              }
          ...
          }).observe($stickyEl.is(this.DOM.$element) ? this.DOM.element : $stickyEl[0]);
        });
        promises.push(promise);
      });
    ...
    initDynamicColors() {
      ...
      const sectionsDetails = LiquidSectionsDetails.getInstance();
      sectionsDetails.getDetails().then(lqdSections => {
...
jQuery(document).ready(function ($) {
  $liquidContents.length && !$liquidBody.hasClass('header-style-side') && !$liquidContents[0].hasAttribute('data-liquid-stack') && $('[data-sticky-header]').liquidStickyHeader();
});
(function ($) {
  'use strict';
...
(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      const sectionDetails = LiquidSectionsDetails.getInstance();
      sectionDetails.getDetails().then(sections => {
        $(sections).imagesLoaded(this._init(sections));
      });
    }

    _init(liquidSections) {
      const sections = liquidSections.filter(sec => !sec.isHidden && !sec.isInFooter).filter(sec => !sec.isInnerSection && !sec.isBgTransparent);
...
(function ($) {
  ...
  Plugin.prototype = {
    ...
    wrapItems() {
      !liquidIsElementor && this.$carouselEl.children('p, style').insertBefore(this.$carouselEl);
    ...
    doSomethingCrazyWithFilter() {
      ...
      const header = $('.lqd-pf-carousel-header').get(0);
      const {
        x
      } = this.flickityData;
...
(function ($) {
  ...
  Plugin.prototype = {
    ...
    formatCounterAnimator: function (number) {
      return number.toString().replace(/(\d)/g, '<span class="lqd-counter-animator"><span class="lqd-animator-value">$1</span></span>');
    },
    createMarkup: function () {
      ...
      counter.find('.lqd-counter-animator').each(function (i, animator) {
        ...
        $animator.append(`<div class="lqd-animator-numbers" data-value="${animatorValue}">
	<ul>
		<li>0</li>
		<li>1</li>
		<li>2</li>
  ...
  class LqdGMap {
    ...
    build() {
      const mapOpts = { ...this.options.map,
        styles: this.styles[this.options.style]
      };
...
(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      const sectionsDetails = LiquidSectionsDetails.getInstance();
      sectionsDetails.getDetails().then(sections => {
    ...
    addClassnames() {
      ...
      $liquidHtml.add('html').addClass('overflow-hidden');
    ...
    init() {
      ...
      if (this.$element.is('video') && this.inlineVideoOptions.disableOnMobile && isMobile) {
        ...
        if (typeof MediaElementPlayer === 'undefined') {
          throw Error('MediaElement js is not defined');
        } else {
...
(function ($) {
  ...
  class Plugin {
    constructor(element, options) {
      ...
      this.DOM.$snickersBarTemp = $('#lqd-snickersbar');
...
(function ($) {
  ...
  let defaults = {
    ...
    itemsSelector: '.lqd-section-scroll-sections > .vc_row'
  };
  ...
  class Plugin {
    ...
    _onUnitsAnimationsComplete(element) {
      ...
      if (element.classList.contains('split-inner')) {
        element.parentElement.classList.remove('lqd-unit-animation-done');
      } else {
        element.classList.remove('lqd-unit-animation-done');
      }
    }
...
jQuery(document).ready(function ($) {
  ...
  const anims = $('[data-custom-animations]').filter((i, element) => {
    ...
    return (!$liquidContents[0].hasAttribute('data-liquid-stack') || $liquidContents[0].hasAttribute('data-liquid-stack') && stackDisableOnMobile && liquidIsMobile()) && !$element.hasClass('carousel-items');
  }).get().reverse();
